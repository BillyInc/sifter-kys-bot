...
class RealTimeMonitor {
  ...
  async start(wallets, onSignal) {
    this.top15Wallets = wallets;
    this.elite15Addresses = new Set(wallets.map(w => w.wallet_address));
    this.onSignalCallback = onSignal;
    this.isRunning = true;

    this.MIN_BUY_USD = parseFloat(await DatabaseService.getSetting('min_buy_usd')) || 100;
    this.BUFFER_WINDOW = parseInt(await DatabaseService.getSetting('signal_window_seconds')) * 1000 || 15000;
  ...
  connectWebSocket() {
    try {
      ...
      this.ws.onopen = () => {
        ...
        this.top15Wallets.forEach(wallet => {
  ...
  async fetchRecentTrades(wallet, secondsBack) {
    try {
  ...
  isElite15Wallet(walletAddress) {
    return this.elite15Addresses.has(walletAddress);
  }

  emitSignal(token, signalData) {
