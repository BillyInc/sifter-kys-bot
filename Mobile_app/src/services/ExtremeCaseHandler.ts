...
interface ExtremeThresholds {
  volatility: number;
  liquidityCrash: number;
  networkCongestion: number;
  multiWalletSignal: number;
  timePressure: number;
}

interface ExtremeMultipliers {
  slippage: { max: number; panic: number };
  gas: { urgent: number; panic: number };
}

interface ExtremeContext {
  walletCount?: number;
  isSell?: boolean;
  priceChange1m?: number;
  networkCongestion?: number;
  liquidity?: number;
  nextTPDistance?: number | null;
}
...
class ExtremeCaseHandler {
  ...
  private baseGas: number;
  private extremeThresholds: ExtremeThresholds;
  private extremeMultipliers: ExtremeMultipliers;
  ...
  async calculateExtremeSlippage(token: string, action: string, context: ExtremeContext): Promise<ExtremeAnalysis> {
    const {
  ...
  getRecommendedAction(score: number, isSell: boolean): string {
  ...
  async getLiquidityChange(token: string): Promise<number> {
    try {
  ...
  async detectWhaleActivity(token: string): Promise<WhaleActivity> {
    try {
      const res = await fetch(`https://api.solana-tracker.io/whales/${token}?minutes=5`);
      const data = await res.json();
      const largeTxs = (data.transactions || []).filter((tx: any) => tx.usdValue > 10000);
      if (!largeTxs.length) return { detected: false };
      const buys = largeTxs.filter((t: any) => t.type === 'buy');
      const sells = largeTxs.filter((t: any) => t.type === 'sell');
      return {
        detected: true,
        type: buys.length > sells.length ? 'buying' : 'selling',
        amount: largeTxs.reduce((s: number, t: any) => s + t.usdValue, 0)
      };
    } catch { return { detected: false }; }
  }

  async detectSnipers(token: string): Promise<number> {
    try {
  ...
  async calculateMEVRisk(token: string): Promise<number> {
    try {
      const apiKey = await DatabaseService.getSetting('api_key_helius');
