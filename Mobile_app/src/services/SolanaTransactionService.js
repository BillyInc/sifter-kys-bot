...
class SolanaTransactionService {
   ...
   */
  async getSwapQuote(inputMint, outputMint, amountLamports, slippageBps = 100) {
   ...
   */
  async buyToken(privateKeyBase58, tokenAddress, amountSol) {
    try {
   ...
   */
  async sellToken(privateKeyBase58, tokenAddress, tokenAmount) {
    try {
      const keypair = Keypair.fromSecretKey(bs58.decode(privateKeyBase58));
   ...
   */
  async sendJitoBundle(serializedTransaction) {
    try {
      const bundlePayload = {
        jsonrpc: '2.0',
        id: 1,
   ...
   */
  async sendWithMevProtection(serializedTransaction) {
