...
const SettingsScreen = ({ navigation }) => {
  ...
  const [portfolio, setPortfolio] = useState(portfolioTotal.toString());
  const [minBuy, setMinBuy] = useState(settings.minBuyUsd?.toString() || '100');
  const [autoReplace, setAutoReplace] = useState(settings.autoReplaceWallets ?? true);
  ...
  const handleSavePortfolio = async () => {
    ...
    await updateSettings({ portfolio_total: val });
  ...
  const Row = ({ label, children, borderBottom = true }) => (
    ...
    <ScrollView style={styles.container}>
      ...
      <Section title="Trade Settings">
        ...
        <Row label="Auto-replace wallets" borderBottom={false}>
          <Switch value={autoReplace} onValueChange={(v) => { setAutoReplace(v); updateSettings({ autoReplaceWallets: v }); }} trackColor={{ true: '#6366f1' }} />
        </Row>
      </Section>
