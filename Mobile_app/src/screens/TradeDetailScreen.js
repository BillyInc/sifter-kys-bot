...
const TradeDetailScreen = ({ route, navigation }) => {
  ...
  const totalSold = history.filter(h => h.action.startsWith('sell')).reduce((s, h) => s + h.usd_value, 0);
  const costBasis = history.find(h => h.action === 'buy')?.usd_value || trade.entry_size;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {trade.triggering_wallets.map((w, i) => (
