...
interface Elite15ScreenProps {
  navigation: any;
}

const Elite15Screen: React.FC<Elite15ScreenProps> = ({ navigation }) => {
  ...
  const handleToggleWatchlist = (wallet: any) => {
    const inList = watchlist.some((w: any) => w.wallet_address === wallet.wallet_address);
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      ...
      {elite15.map((wallet: any) => {
        const inList = watchlist.some((w: any) => w.wallet_address === wallet.wallet_address);
        const listStatus = watchlist.find((w: any) => w.wallet_address === wallet.wallet_address)?.status;
