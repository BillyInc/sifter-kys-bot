...
describe('PositionSizeManager', () => {
  ...
  beforeEach(() => {
    ...
    manager.portfolioSize = 10000;
    manager.tradingBalance = 5000;
  });

  test('calculates position size for single wallet signal', () => {
    const size = manager.calculateSize({
      token: 'TOKEN1',
      wallets: ['wallet1'],
      strength: 'normal',
    });
    // First signal from 1 wallet = 30% of trading balance
    expect(size).toBeLessThanOrEqual(manager.tradingBalance * 0.4);
  ...
  test('never exceeds 40% per token', () => {
    ...
    const remaining = manager.calculateSize({
      token: 'TOKEN1',
      wallets: ['w1', 'w2', 'w3'],
      strength: 'mega',
    });
    const total = 3000 + remaining;
    expect(total).toBeLessThanOrEqual(manager.portfolioSize * 0.4);
  });
});
