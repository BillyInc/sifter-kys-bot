"""
RQ Worker Tasks - On-demand analysis and data fetching
These are triggered by user actions (analyze button, etc.)
Runs on RQ workers, not Celery
"""
...
def analyze_single_token_parallel(token, user_id, parent_job_id):
    ...
    job1 = q.enqueue('worker_tasks.fetch_top_traders', {
        'token': token,
    ...
    job2 = q.enqueue('worker_tasks.fetch_first_buyers', {
        'token': token,
        'parent_job_id': parent_job_id
    })
    ...
    job3 = q.enqueue('worker_tasks.fetch_birdeye_trades', {
        'token': token,
        'parent_job_id': parent_job_id
    })
    ...
    job4 = q.enqueue('worker_tasks.fetch_recent_trades', {
        'token': token,
        'parent_job_id': parent_job_id
    })
    ...
    for i in range(0, len(wallet_list), batch_size):
        ...
        job = q.enqueue('worker_tasks.fetch_pnl_batch', {
            'wallets': batch,
            'token': token,
    ...
    for i in range(0, len(qualified_wallets), batch_size):
        ...
        job = q.enqueue('worker_tasks.fetch_runner_history_batch', {
            'wallets': batch,
            'token': token,
            'parent_job_id': parent_job_id
        })
...
def analyze_multiple_tokens_parallel(tokens, user_id, parent_job_id):
    ...
    token_jobs = []
    for token in tokens:
        job = q.enqueue('worker_tasks.analyze_single_token_parallel', 
    ...
    for addr, data in wallet_hits.items():
        ...
        ranked.append({
            ...
            'analyzed_tokens': data['tokens_hit'],
...
def preload_trending_cache_parallel():
    ...
    jobs.append(q.enqueue('worker_tasks.warm_cache_7d'))
    
    # Worker 2: 14-day runners
    jobs.append(q.enqueue('worker_tasks.warm_cache_14d'))
    
    # Worker 3-5: Analyze top runners from each timeframe
    jobs.append(q.enqueue('worker_tasks.analyze_top_runners', {'days': 7, 'count': 3}))
    jobs.append(q.enqueue('worker_tasks.analyze_top_runners', {'days': 14, 'count': 3}))
    jobs.append(q.enqueue('worker_tasks.analyze_top_runners', {'days': 30, 'count': 3}))
