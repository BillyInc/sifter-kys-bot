    ...
    <>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <button onClick={() => setShowToast(false)} className="ml-auto p-1 hover:bg-white/10 rounded transition">
                    <X size={14} />
                  </button>
                </div>
                <p className="text-sm text-gray-300 mb-2">
                  ...
                  {' '}just{' '}
                  <span className={latestNotification.side === 'buy' ? 'text-green-400' : 'text-red-400'}>{latestNotification.side}</span>
                  {' '}<span className="font-bold text-white">{formatUSD(latestNotification.usd_value)}</span>
                  {' '}of{' '}<span className="text-yellow-400">${latestNotification.token_ticker}</span>
                </p>
                <button onClick={() => { setShowToast(false); setIsOpen(true); }} className="text-xs text-purple-400 hover:text-purple-300 transition flex items-center gap-1">
      ...
      <div className="relative" ref={dropdownRef}>
        <button onClick={() => setIsOpen(!isOpen)} className="relative p-2 hover:bg-white/10 rounded-lg transition">
          {unreadCount > 0 ? <BellRing className="text-purple-400 animate-pulse" size={20} /> : <Bell className="text-gray-400" size={20} />}
          {unreadCount > 0 && (
          ...
          <div className="absolute right-0 top-12 w-96 max-h-[600px] bg-black border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-fade-in z-50">
            <div className="bg-gradient-to-r from-purple-900/50 to-purple-800/30 border-b border-white/10 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setIsOpen(false)} className="p-1 hover:bg-white/10 rounded transition"><X size={18} /></button>
              </div>
              {unreadCount > 0 && (
                <button onClick={handleMarkAllAsRead} className="text-xs text-purple-400 hover:text-purple-300 transition flex items-center gap-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <div>
                              ...
                              <span className={`ml-2 text-sm font-bold ${notification.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>{notification.side.toUpperCase()}</span>
                            </div>
                            {!notification.read_at && <div className="w-2 h-2 bg-purple-500 rounded-full flex-shrink-0" />}
                          </div>
                          <p className="text-sm text-gray-400 mb-1">
                            <span className="text-yellow-400 font-semibold">${notification.token_ticker}</span>
                            {notification.token_name && <span className="text-xs text-gray-500 ml-1">({notification.token_name})</span>}
                          </p>
                          <div className="flex items-center gap-3 text-xs text-gray-500">
                            <span className="font-bold text-white">{formatUSD(notification.usd_value)}</span>
