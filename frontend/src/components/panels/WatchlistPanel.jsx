...
export default function WatchlistPanel({ 
  ...
  const handleRefreshWallet = async (walletAddress) => {
    try {
      ...
      if (data.success) {
        // Reload full watchlist to get updated stats
        await loadWatchlist();
      }
    } catch (error) {
  ...
  const warningCount = wallets.filter(w => w.degradation_alerts && w.degradation_alerts.some(a => a.severity === 'yellow')).length;
  const criticalCount = wallets.filter(w => w.degradation_alerts && w.degradation_alerts.some(a => a.severity === 'red')).length;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Premier League Table with Expandable Cards */}
